      ...
      <div className="container-page relative grid min-h-[calc(100vh-73px)] content-center gap-12 py-16 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="max-w-3xl">
          <div className="mb-5 flex flex-wrap gap-3">
            <span className="inline-flex items-center gap-2 rounded-full border border-dioza-gold/50 bg-white/10 px-4 py-2 text-sm font-black text-dioza-gold backdrop-blur">
              <ShieldCheck size={16} />
              {settings.halalText}
            </span>
          ...
          <p className="mt-6 max-w-2xl text-xl leading-8 text-white/90 sm:text-2xl">
            Des ingrédients frais, un service rapide et une cuisine halal préparée avec soin.
          </p>

          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <a href={settings.phoneHref} className="btn-primary text-base">
              <Phone size={20} />
              Commander {settings.phone}
            </a>
            {settings.platforms.map((platform) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className={`${platform.color} inline-flex items-center justify-center rounded-full px-6 py-3 text-sm font-black uppercase tracking-wide text-white shadow-xl transition hover:-translate-y-0.5`}
              >
                {platform.name}
              </a>
            ))}
          ...
          <div className="mt-10 grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-white/15 bg-white/10 p-4 backdrop-blur">
              ...
              <p className="font-black">{settings.hours}</p>
            </div>
            <div className="rounded-2xl border border-white/15 bg-white/10 p-4 backdrop-blur">
              <MapPin className="mb-3 text-dioza-gold" />
              <p className="font-black">{settings.address}</p>
            </div>
          </div>
        </div>
        ...
        <div className="hidden lg:flex lg:items-end lg:justify-end">
          <div className="card max-w-md bg-white/95 p-6 text-dioza-deep">
            ...
            <div className="mt-5 grid gap-3">
              {[settings.halalText, 'Livraison rapide', 'Disponible sur Deliveroo & Uber Eats'].map((item) => (
